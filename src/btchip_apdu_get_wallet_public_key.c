...
int get_public_key_chain_code(unsigned char* keyPath, bool uncompressedPublicKeys, unsigned char* publicKey, unsigned char* chainCode, bool btcv3KeysAddr) {
    cx_ecfp_private_key_t private_key;
    cx_ecfp_public_key_t public_key;
    int keyLength = 0;
 
    if(btcv3KeysAddr && G_io_apdu_buffer[ISO_OFFSET_P2] == P2_BTCV_3KEYS_ADDR_INSTANT_PUBKEY)
        btchip_private_derive_keypair(G_io_apdu_buffer + ISO_OFFSET_CDATA, 1, chainCode, &private_key, &public_key, Instant);
    else if(btcv3KeysAddr && G_io_apdu_buffer[ISO_OFFSET_P2] == P2_BTCV_3KEYS_ADDR_RECOVERY_PUBKEY)
        btchip_private_derive_keypair(G_io_apdu_buffer + ISO_OFFSET_CDATA, 1, chainCode, &private_key, &public_key, Recovery);
    else
        btchip_private_derive_keypair(G_io_apdu_buffer + ISO_OFFSET_CDATA, 1, chainCode, &private_key, &public_key, Regular);
...
unsigned short btchip_apdu_get_wallet_public_key() {
    ...
    bool btcv3KeysAddr = (G_io_apdu_buffer[ISO_OFFSET_P2] == P2_BTCV_3KEYS_ADDR_REGULAR_PUBKEY || G_io_apdu_buffer[ISO_OFFSET_P2] == P2_BTCV_3KEYS_ADDR_INSTANT_PUBKEY || G_io_apdu_buffer[ISO_OFFSET_P2] == P2_BTCV_3KEYS_ADDR_RECOVERY_PUBKEY);
    ...
    case P2_CASHADDR:
        ...
        break;
    case P2_BTCV_3KEYS_ADDR_REGULAR_PUBKEY:
    case P2_BTCV_3KEYS_ADDR_INSTANT_PUBKEY:
    case P2_BTCV_3KEYS_ADDR_RECOVERY_PUBKEY:
    ...
    keyLength = get_public_key_chain_code(G_io_apdu_buffer + ISO_OFFSET_CDATA, uncompressedPublicKeys, G_io_apdu_buffer + 1, chainCode, btcv3KeysAddr);
